...
class Traffic():
    ...
    def generate_intersection(self):
        ...
        for i in range(self.total_intersections):
            ...
            streets_in = deepcopy(list(self.street_detail.loc[self.street_detail['end_int'] == i, 'name']))
    ...
    #                     print("{} has reached the destination and received {} points.".format(car_to_new_street, car_score))
            ...
            if _callback:
                ...
                _callback(T, deepcopy(self.cars), deepcopy(self.streets), deepcopy(self.street_detail), deepcopy(self.intersections))
    ...
    def generate_submission_file(self, intersection_schedule_list, out_file_path='submit.example.txt'):
        with open(out_file_path, 'w') as out_file:
            ...
            for intersection_schedule in intersection_schedule_list:
                out_file.write("{}\n{}\n".format(intersection_schedule['id'], intersection_schedule['num_incoming_streets']))
                for order_duration_green_light in intersection_schedule['order_duration_green_lights']:
                    out_file.write("{} {}\n".format(order_duration_green_light['street_name'], order_duration_green_light['green_seconds']))
